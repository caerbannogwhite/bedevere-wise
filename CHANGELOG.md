...
- [Feature] **CodeMirror multi-cursor + find.** `Ctrl+D` extends the selection to the next occurrence of the currently selected text (classic VS Code / Sublime multi-edit). `Alt+ArrowUp` / `Alt+ArrowDown` drop a cursor above / below the current line. `Ctrl+F` opens the editor's find panel; F3 / Shift+F3 step through matches.
